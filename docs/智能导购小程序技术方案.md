...
MVP 采用“微信小程序 + 模块化单体后端 + PostgreSQL/pgvector + 外部大模型”的架构。`backend/bff` 负责登录、商家、会话、消息、AI 导购和内部管理接口；其中 `src/ai` 领域模块负责商品导入后的 Embedding、混合检索、候选商品约束和回复生成。
...
AI 能力作为 `backend/bff/src/ai` 内部模块直接调用，不通过网络回调自身形成伪微服务。
